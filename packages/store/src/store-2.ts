...
type ActionsContext<TState, TActions, TSelectors, TEvents> = (store: {
  ...
  clearCache: (selectorName: keyof TSelectors | (keyof TSelectors)[]) => void; // Add clearCache
}) => TActions;

type SelectorsContext<TState, TSelectors> = (store: {
  states: TState;
  selectors: TSelectors;
  cache: <T>(fn: () => T) => CacheWrapper<T>;
}) => {
...
> = {
  ...
  cacheSelectors?: readonly (keyof TSelectors)[]; // Keep existing but will be auto-detected
  config?: {
    name?: string;
    devtools?: boolean;
  };
...
type EventMap = Map<string, Set<EventCallback<AnyType>>>;
type EventPayload<
  TEvents,
...
const createBatcher = () => {
  ...
  let batchedActions: { type: string; payload: AnyType }[] = [];
  ...
  const addBatchedAction = (action: { type: string; payload: AnyType }) => {
    if (isBatching) {
...
function createReactiveProxy<T>(
  ...
  function wrap(value: AnyType): AnyType {
...
export function deepClone<T>(obj: T): T {
  ...
  if (obj instanceof Date) {
    return new Date(obj.getTime()) as AnyType;
  }
  // Handle Array objects
  if (Array.isArray(obj)) {
    return obj.map((item) => deepClone(item)) as AnyType;
  }
  // Handle Map objects
...
// Add cache function helper
const cache = <T>(fn: () => T): CacheWrapper<T> => ({
  __isCached: true,
  fn
});

// Helper to check if a value is a cache wrapper
...
>(
  ...
  // Process selectors to detect cached ones automatically
  const selectorProxy = {} as TSelectors;
  const cachedSelectorNames = new Set<string>();

  // Initialize selectors first to detect cached ones
  if (props.selectors) {
    const tempSelectors = props.selectors({
      states,
      selectors: selectorProxy,
      cache // Add cache function to context
    });

    // Detect which selectors are cached by examining the returned object
    ...
    // Also include manually specified cached selectors for backward compatibility
    if (props.cacheSelectors) {
      props.cacheSelectors.forEach((name) =>
        cachedSelectorNames.add(String(name))
      );
    }
  }
  ...
  function createSelectorCacheKey(
    selectorName: string,
    args: AnyType[]
  ): string {
  ...
  function updateGlobalSelectorCaches() {
      ...
      if (!cached || cached.stateVersion !== currentStateVersion) {
        try {
          if (selectors[selectorName as keyof TSelectors]) {
            ...
            const newResult = (
              selectors[selectorName as keyof TSelectors] as AnyType
            )(...args);
    ...
    if (typeof window !== 'undefined' && (window as AnyType).__STORE_DEBUG__) {
      console.log(
  ...
  let pendingDevToolsActions: { type: string; payload: AnyType }[] = [];
  ...
  // Invalidate and clearCache functions for cached selectors
  function invalidate(selectorName: keyof TSelectors | (keyof TSelectors)[]) {
    return clearCacheInternal(selectorName);
  }

  function clearCacheInternal(
    selectorName: keyof TSelectors | (keyof TSelectors)[]
  ) {
    ...
    if (Array.isArray(selectorName)) {
      ...
      selectorName.forEach((name) => {
        ...
        if (isCachedSelector(selectorNameStr)) {
          ...
          if (selectors[name]) {
            try {
              const baseResult = (selectors[name] as AnyType)();
    ...
    if (selectors[selectorName]) {
      try {
        const baseResult = (selectors[selectorName] as AnyType)();
        const baseCacheKey = createSelectorCacheKey(selectorNameStr, []);
  ...
  const actions = props.actions
    ? props.actions({
        ...
        invalidate,
        clearCache: clearCacheInternal // Add clearCache to actions context
      })
    : ({} as TActions);

  // Now create the actual selectors, processing cache wrappers
  const selectors = {} as TSelectors;
  if (props.selectors) {
    const rawSelectors = props.selectors({
      states: statesProxy,
      selectors: selectorProxy,
      cache
    });

    // Process selectors, unwrapping cache wrappers
    for (const [key, selector] of Object.entries(rawSelectors)) {
      if (isCacheWrapper(selector)) {
        // Store the unwrapped function
        selectors[key as keyof TSelectors] = selector.fn as AnyType;
      } else {
        selectors[key as keyof TSelectors] = selector;
      }
    }
  }
  ...
  function scheduleDevTools(actionInfo: { type: string; payload: AnyType }) {
  ...
  const createDispatchObject = () =>
    Object.keys(actions).reduce((acc, actionKey) => {
      acc[actionKey] = (...args: AnyType[]) => {
        ...
        const result = (cb as AnyType)(...args);
  ...
  const wrappedGetSelectors = Object.keys(selectors).reduce(
    (acc, key) => {
      ...
      acc[key] = function (...args: AnyType[]) {
        ...
        const result = (selectors[key as keyof TSelectors] as AnyType)(...args);
    ...
    (acc, key) => {
      ...
      acc[key] = function (...args: AnyType[]) {
        ...
        const wrappedSelector = (_state: TState) => {
          ...
          const result = (selectors[key as keyof TSelectors] as AnyType)(
            ...args
          );
